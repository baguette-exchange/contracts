...
contract BaguetteRouter is IBaguetteRouter {
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'BaguetteRouter: EXPIRED');
        _;
    }
    ...
    function _addLiquidity(
        ...
        if (IBaguetteFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            IBaguetteFactory(factory).createPair(tokenA, tokenB);
        }
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'BaguetteRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = BaguetteLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'BaguetteRouter: INSUFFICIENT_A_AMOUNT');
    ...
    function removeLiquidity(
        ...
        address pair = BaguetteLibrary.pairFor(factory, tokenA, tokenB);
        IBaguettePair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
        (uint amount0, uint amount1) = IBaguettePair(pair).burn(to);
        (address token0,) = BaguetteLibrary.sortTokens(tokenA, tokenB);
        (amountA, amountB) = tokenA == token0 ? (amount0, amount1) : (amount1, amount0);
        require(amountA >= amountAMin, 'BaguetteRouter: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'BaguetteRouter: INSUFFICIENT_B_AMOUNT');
    }
    ...
    function removeLiquidityWithPermit(
        ...
        IBaguettePair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            IBaguettePair(BaguetteLibrary.pairFor(factory, input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'BaguetteRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, 'BaguetteRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, BaguetteLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, to);
    ...
    {
        require(path[0] == WAVAX, 'BaguetteRouter: INVALID_PATH');
    ...
    function swapTokensForExactAVAX(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WAVAX, 'BaguetteRouter: INVALID_PATH');
        amounts = BaguetteLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'BaguetteRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, BaguetteLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WAVAX, 'BaguetteRouter: INVALID_PATH');
        amounts = BaguetteLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, 'BaguetteRouter: EXCESSIVE_INPUT_AMOUNT');
        IWAVAX(WAVAX).deposit{value: amounts[0]}();
        assert(IWAVAX(WAVAX).transfer(BaguetteLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust AVAX, if any
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = BaguetteLibrary.sortTokens(input, output);
            IBaguettePair pair = IBaguettePair(BaguetteLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            ...
            'BaguetteRouter: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
